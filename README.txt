...
Linux
First, you'll need to install the OpenCV framework. If your package manager doesn't have it, you can compile it from the latest opencv*-*.tar.gz at http://sourceforge.net/projects/opencvlibrary/files/. Then, you'll need to get the Java bindings for OpenCV from http://ubaa.net/shared/processing/opencv/, and drag the libOpenCV.so file to somewhere in your Java Library Path. Finally, you should make sure that Gypsum.jar is run with a 32-bit VM, since the Java bindings for OpenCV are 32-bit, and cannot be loaded from a 64-bit VM.
